...
Shiny 是基于mmap的linux轻量级日志模块, 目前仅支持linux环境, Android、Windows等环境暂不支持。它的核心是通过mmap对磁盘文件进行一一映射, 只进行映射而没有任何文件的IO、用户态内核态之间的数据拷贝操作, 基于mmap进行文件的读写与内存读写的性能是一样的, 保证了日志模块的的高性能。
